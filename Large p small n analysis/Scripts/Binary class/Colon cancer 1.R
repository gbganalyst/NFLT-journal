...
dataset=read.xlsx('Large p small n dataset/Binary class/colon-cancer-1.xlsx',sheet = 1)
...
err %>% gather(1:13,key = 'Model',value = 'AVTE') %>% ggplot(.,aes(reorder(x=Model,AVTE, median), y=AVTE, fill=Model))+geom_boxplot(show.legend = F)+theme_bw()+labs(y=' Average test error', x='Method (Classifier)', title = 'Average test error of each classifier',caption = "Source: colon-cancer-1 dataset")+theme(axis.title.x = element_text(face = 'bold',size = 12),axis.title.y = element_text(face = 'bold',size = 12),axis.text.x = element_text(angle = 50, vjust = 0.5))->AVTE_colon_cancer_1                                           

ggsave("Large p small n script/Images of average test errors/Binary class/AVTE_colon_cancer_1.png",width = 6.74, height = 4.54)
...
data.frame(SN=1:13, AVTE=avg.err) %>%
  ...
  labs(y="Average prediction error", x="Method (Classifier)", title = "Average prediction error of each classifier", caption = "Source: colon-cancer-1 dataset")+ theme(axis.title.x = element_text(face = "bold",size = 12),axis.title.y = element_text(face = "bold",size = 12))->AVPE_colon_cancer_1

ggsave("Large p small n script/Images of average test errors/Binary class/AVPE_colon_cancer_1.png",width = 6.74, height = 4.54)
...
write.table(colon_table.rk, "Colon1", sep="\t", row.names = F)
